...
public class Comms{
	...
	static int[] lengthOfEachPath = new int[DataCache.maxPathLength];
	...
	public static ArrayList<MapLocation> downloadPath() throws GameActionException {
		...
		int locationInt = rc.readBroadcast(CowboyRobot.myBand+1);
		while(locationInt>=0){
		...
		CowboyRobot.myBand = -locationInt*DataCache.maxPathLength;
	...
	public static void findPathAndBroadcast(int bandID,MapLocation start, MapLocation goal, int bigBoxSize, int joinSquadNo) throws GameActionException{
		...
		int band = bandID*DataCache.maxPathLength;
